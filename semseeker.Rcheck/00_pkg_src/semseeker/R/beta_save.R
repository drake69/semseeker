signal_save <- function(signal_data, sample_sheet, batch_id )
{
  ...
  signal_data <- rbind(sample_info, signal_data )
  pivot_subfolder <- dir_check_and_create(ssEnv$result_folderData,c("Pivots","SIGNAL"))
  fileName <- file_path_build(pivot_subfolder,c("SIGNAL","MEAN","PROBE","PROBE"),"csv")
  utils::write.table(signal_data, fileName, row.names = T, col.names = T, sep=";")
