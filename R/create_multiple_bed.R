...
create_multiple_bed <- function(sample_sheet){
  ...
  localKeys$COUNT <- 0
  key_updated <- data.frame()
  key_updated <- foreach::foreach(i = 1:nrow(localKeys), .export = to_export, .combine = sum) %dorng%
    # for(i in 1:nrow(localKeys))
    {
      ...
      key_samples <- data.frame()
      if(!file.exists(fileToWrite))
      {
        j <- 0
        # foreach::foreach(j = 1:nrow(sample_sheet), .export = to_export) %dorng%
        for ( j in 1:nrow(sample_sheet))
        {
          ...
          if(file.exists(fileToRead))
          {
            ...
            key$COUNT <- max(key$COUNT, nrow(localtemp))
            key$Sample_ID <- sample$Sample_ID
            key_samples <- plyr::rbind.fill(key_samples, key)
      ...
      # key_updated <- plyr::rbind.fill(key_updated, key_samples)
      # key_samples
      gc()
      1
    }
  ...
  gc()
  # key_updated <- unique(key_updated)
  # key_updated$COMBINED <- paste(key_updated$MARKER, key_updated$FIGURE, sep = "_")
  #
  # key_updated <- reshape2::dcast(data = key_updated, formula =  Sample_ID + SAMPLE_GROUP ~ COMBINED, value.var = "COUNT",
  #   fun.aggregate = sum, drop = TRUE)
  # # write the key file
  # fileToWrite <- file_path_build(ssEnv$result_folderData, c("SAMPLES_AREA", "SUMMARY"), "csv", add_gz=TRUE)
  # utils::write.table(key_updated, gzfile(fileToWrite), sep = "\t", row.names = FALSE, col.names = TRUE, quote = FALSE, append = FALSE)
