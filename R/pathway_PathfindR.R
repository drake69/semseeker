      ...
      try(
        {
          ...
          output_temp <- pathfindR::run_pathfindR( gene_set , path_db[k], max_gset_size = nrow(gene_set), iterations = 10,
                                                   output_dir = paste(paste(resultFolder,"/Pathway/","finder_", sep=""), seq, sep=""))
          seq <- seq + 1
          ...
          write.csv(pathway_result, paste(resultFolder,"/Pathway/",gsub("[.]","",pvalue),"_",applied_model,"pathway_result.csv", sep=""))
        }
      )
    }
  }
