coverage_analysis <- function(methylation_data, envir)
{
  ...
  number_areas <- aggregate(temp_cov_result$COUNT, list(temp_cov_result$GROUP,temp_cov_result$SUBGROUP), FUN=sum)
  ...
  ggp <- ggplot2::ggplot(temp_cov_result, ggplot2::aes(GENOMIC_AREA, COV_PERC )) +    # Create default ggplot2 heatmap
    ggplot2::geom_tile(ggplot2::aes(fill = AREA_PERC)) +
    ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, vjust = 0.5, hjust=1)) +
    ggplot2::geom_text(ggplot2::aes(label = AREA_PERC)) +
  ...
  temp_cov_result <- subset(cov_result,SUBGROUP !="Whole" & SUBGROUP !="CHR")
  ...
  h_total <- temp_cov_result %>%
    dplyr::group_by(GENOMIC_AREA) %>%
    dplyr::summarise(AREA_PERC = sum(AREA_PERC)) %>%
    dplyr::mutate(COV_PERC = 'Total')
  v_total <- temp_cov_result %>%
    dplyr::group_by(COV_PERC) %>%
    dplyr::summarise(AREA_PERC = sum(AREA_PERC)) %>%
    dplyr::mutate(GENOMIC_AREA = 'Total')

  ggp <- ggplot2::ggplot(temp_cov_result, ggplot2::aes( x =GENOMIC_AREA, y=COV_PERC )) +    # Create default ggplot2 heatmap
    ggplot2::geom_tile(ggplot2::aes(fill = AREA_PERC)) +
    ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 90, vjust = 0.5, hjust=1)) +
    ggplot2::geom_text(ggplot2::aes(label = AREA_PERC), size=3) +
    ggplot2::scale_fill_gradient(low = "white", high = "#1b98e0") +
    ggplot2::labs(x = "Genomic Area", y = " Percentage of covered Probes") +
    ggplot2::labs(fill  = "Percentage\nover\nwhole\ngenome", color="% Covered\narea\nover\nstudied") +
    ggplot2::geom_point(data = h_total, ggplot2::aes(color = AREA_PERC), size = 10, shape = 19) +
    ggplot2::geom_point(data = v_total, ggplot2::aes(color = AREA_PERC), size = 10, shape = 19) +
    ggplot2::scale_color_gradient2(low = "white",high = "grey",midpoint = 0) +
    ggplot2::geom_text(data = h_total, size = 3, ggplot2::aes(label = round(AREA_PERC,2))) +
    ggplot2::geom_text(data = v_total, size = 3, ggplot2::aes(label = round(AREA_PERC,2)))
